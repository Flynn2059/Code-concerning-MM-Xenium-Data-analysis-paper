...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 20 14:43:07 2026
...
#%% 读取一个分组的邻域矩阵
MM_10_samples = pd.read_csv('./neighborhood_results/MM_50um_pct_matrix.csv', index_col=0)
MM_10_samples = MM_10_samples[
    MM_10_samples["celltype"].isin(["CD4+ T Naive", "pct_CD8+ T Effector",
                                     'pct_CD8+ T Exhausted','pct_T Proliferating']) &
    (MM_10_samples["n_neighbors"] > 5)
].copy()

print(MM_10_samples["sample"].value_counts())

# 提取 pct 列
pct_cols = [col for col in MM_10_samples.columns if col.startswith("pct_")]

# 获取样本列表
sample_names = sorted(MM_10_samples["sample"].unique().tolist())
print(sample_names)
#%% 绘制特征分布的直方图
...
for sample_name in sample_names:
    ...
    sample_df = MM_10_samples[MM_10_samples["sample"] == sample_name].copy()
    ...
    for k in k_values:
        ...
        for res in resolutions:
            ...
            print(
                ...
                f"MMallest={int(cluster_sizes.iloc[-1])}"
            )
...
for k in selected_k:
    fig, axes = plt.subplots(2, 5, figsize=(55, 18), dpi=600)
    axes = axes.flatten()
    ...
    out_path = os.path.join(
        plot_dir,
        f"MM_50um_nmf_H_heatmap_k{k}_by_sample.svg"
    )
