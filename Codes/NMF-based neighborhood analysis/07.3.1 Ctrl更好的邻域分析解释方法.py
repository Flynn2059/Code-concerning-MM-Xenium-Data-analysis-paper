...
#%% 读取一个分组的邻域矩阵
ctrl_4_samples = pd.read_csv('./neighborhood_results/Ctrl_50um_pct_matrix.csv', index_col=0)
ctrl_4_samples = ctrl_4_samples[
    ctrl_4_samples["celltype"].isin(["CD4+ T", "CD8+ T"]) &
    (ctrl_4_samples["n_neighbors"] > 5)
].copy()

print(ctrl_4_samples["sample"].value_counts())

# 提取 pct 列
pct_cols = [col for col in ctrl_4_samples.columns if col.startswith("pct_")]

# 获取样本列表
sample_names = sorted(ctrl_4_samples["sample"].unique().tolist())
print(sample_names)
#%% 绘制特征分布的直方图
...
for sample_name in sample_names:
    ...
    sample_df = ctrl_4_samples[ctrl_4_samples["sample"] == sample_name].copy()
...
for k in selected_k:
    ...
    out_path = os.path.join(
        plot_dir,
        f"Ctrl_50um_nmf_H_heatmap_k{k}_by_sample.svg"
    )
