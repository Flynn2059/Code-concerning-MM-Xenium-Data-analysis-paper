...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 20 14:37:12 2026
...
#%% 读取一个分组的邻域矩阵
sm_5_samples = pd.read_csv('./neighborhood_results/SM_50um_pct_matrix.csv', index_col=0)
sm_5_samples = sm_5_samples[
    sm_5_samples["celltype"].isin(["CD4+ T Naive", "pct_CD8+ T Effector",
                                     'pct_T Proliferating']) &
    (sm_5_samples["n_neighbors"] > 5)
].copy()

print(sm_5_samples["sample"].value_counts())

# 提取 pct 列
pct_cols = [col for col in sm_5_samples.columns if col.startswith("pct_")]

# 获取样本列表
sample_names = sorted(sm_5_samples["sample"].unique().tolist())
print(sample_names)
#%% 绘制特征分布的直方图
...
for sample_name in sample_names:
    ...
    sample_df = sm_5_samples[sm_5_samples["sample"] == sample_name].copy()
...
for k in selected_k:
    fig, axes = plt.subplots(2, 3, figsize=(33, 18), dpi=600)
    axes = axes.flatten()
    ...
    out_path = os.path.join(
        plot_dir,
        f"SM_50um_nmf_H_heatmap_k{k}_by_sample.svg"
    )
