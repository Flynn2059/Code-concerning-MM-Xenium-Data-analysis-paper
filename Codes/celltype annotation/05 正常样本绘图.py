...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar  8 22:33:11 2026
...
for sdata_key, sdata in sdata_dict.items():
    sample_name = sdata_key.replace("sdata_", "")
    print(f"\n{sample_name}")
    print(sdata["table"].obs["annotation"].value_counts(dropna=False).head(10))
...
groups = ["Other", "CD4+ T", "CD8+ T"]
palette = ["#BDBDBD", "#00A087FF", "#E64B35FF"]
...
for sdata_key, sdata in sdata_dict.items():
    ...
    adata.obs.loc[adata.obs["annotation"] == "CD4+ T", "Tcell_highlight"] = "CD4+ T"
    adata.obs.loc[adata.obs["annotation"] == "CD8+ T", "Tcell_highlight"] = "CD8+ T"
...
for sample_name in sample_order:
    ...
    cd4_count = (ann == "CD4+ T").sum()
    cd8_count = (ann == "CD8+ T").sum()
...
plt.tight_layout()
plt.savefig(
    os.path.join(outdir, f"Ctrl_T_cell_barplot.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, f"Ctrl_T_cell_barplot.svg"),
    bbox_inches="tight"
)
plt.close()
...
df_plot["CD4_CD8_ratio"] = np.where(
    df_plot["CD8_pct_total"] > 0,
    df_plot["CD4_pct_total"] / df_plot["CD8_pct_total"],
    np.nan
)

# 一列即可：行名是样本名，列是 CD4/CD8 ratio
...
# 保存表格
df_ratio.to_csv(
    os.path.join(outdir_ratio, "Ctrl_CD4_CD8_ratio_table.csv")
)
...
for sdata_key, sdata in sdata_dict.items():
    ...
    # T 细胞 centroid
    t_mask = gdf["annotation"].isin(["CD4+ T", "CD8+ T"])
...
plt.tight_layout()
plt.savefig(
    os.path.join(outdir, "4_ctrl_T_vs_Plasma_2D_KDE_contour_panel.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, "4_ctrl_T_vs_Plasma_2D_KDE_contour_panel.svg"),
    bbox_inches="tight"
)
plt.close()
...
os.makedirs(outdir, exist_ok=True)
sample_order = [
    "sdata_GSM9035023_Ctrl-1",
    "sdata_GSM9035024_Ctrl-2",
    "sdata_GSM9035025_Ctrl-3",
    "sdata_GSM9035026_Ctrl-4",
]

fig, axes = plt.subplots(2, 2, figsize=(22, 18), dpi=600)
...
plt.savefig(
    os.path.join(outdir, "Ctrl_T_cell_spatial_co_localization_plot.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, "Ctrl_T_cell_spatial_co_localization_plot.svg"),
    bbox_inches="tight"
)
plt.close()
