...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar  1 21:09:25 2026
...
zarr_files = [
    "GSM9035029_MM-1_post_qc.zarr",
    "GSM9035030_MM-2_post_qc.zarr",
    "GSM9035031_MM-3_post_qc.zarr",
    "GSM9035032_MM-4_post_qc.zarr",
    "GSM9035033_MM-5_post_qc.zarr",
    "GSM9035034_MM-6_post_qc.zarr",
    "GSM9035035_MM-7_post_qc.zarr",
    "GSM9035036_MM-8_post_qc.zarr",
    "GSM9035037_MM-9_post_qc.zarr",
    "GSM9035038_MM-10_post_qc.zarr",
...
#%% 整合RCTD结果
rctd_mm_1=pd.read_csv("./RCTD/doublet_result_MM1.csv",index_col=0)
rctd_mm_2=pd.read_csv("./RCTD/doublet_result_MM2.csv",index_col=0)
rctd_mm_3=pd.read_csv("./RCTD/doublet_result_MM3.csv",index_col=0)
rctd_mm_4=pd.read_csv("./RCTD/doublet_result_MM4.csv",index_col=0)
rctd_mm_5=pd.read_csv("./RCTD/doublet_result_MM5.csv",index_col=0)
rctd_mm_6=pd.read_csv("./RCTD/doublet_result_MM6.csv",index_col=0)
rctd_mm_7=pd.read_csv("./RCTD/doublet_result_MM7.csv",index_col=0)
rctd_mm_8=pd.read_csv("./RCTD/doublet_result_MM8.csv",index_col=0)
rctd_mm_9=pd.read_csv("./RCTD/doublet_result_MM9.csv",index_col=0)
rctd_mm_10=pd.read_csv("./RCTD/doublet_result_MM10.csv",index_col=0)

rctd_mm_1.index = "GSM9035029_MM-1_" + rctd_mm_1.index.astype(str)
rctd_mm_2.index = "GSM9035030_MM-2_" + rctd_mm_2.index.astype(str)
rctd_mm_3.index = "GSM9035031_MM-3_" + rctd_mm_3.index.astype(str)
rctd_mm_4.index = "GSM9035032_MM-4_" + rctd_mm_4.index.astype(str)
rctd_mm_5.index = "GSM9035033_MM-5_" + rctd_mm_5.index.astype(str)
rctd_mm_6.index = "GSM9035034_MM-6_" + rctd_mm_6.index.astype(str)
rctd_mm_7.index = "GSM9035035_MM-7_" + rctd_mm_7.index.astype(str)
rctd_mm_8.index = "GSM9035036_MM-8_" + rctd_mm_8.index.astype(str)
rctd_mm_9.index = "GSM9035037_MM-9_" + rctd_mm_9.index.astype(str)
rctd_mm_10.index = "GSM9035038_MM-10_" + rctd_mm_10.index.astype(str)

rctd=pd.concat([rctd_mm_1,rctd_mm_2,rctd_mm_3,rctd_mm_4,rctd_mm_5,rctd_mm_6,rctd_mm_7,rctd_mm_8,rctd_mm_9,rctd_mm_10],axis=0)
# 把RCTD的结果写回去
...
#%% 整合banksy结果
banksy_mm_pca=pd.read_csv("./MM_10_samples_PCA.csv",index_col=0)
banksy_mm_umap=pd.read_csv("./MM_10_samples_UMAP.csv",index_col=0)
banksy_mm_leiden=pd.read_csv("./MM_10_samples_leiden_clustering.csv",index_col=0)


adata.obsm['X_banksy_pca'] = banksy_mm_pca.values.astype(np.float32) 
adata.obsm['X_banksy_umap'] = banksy_mm_umap.values.astype(np.float32)
adata.obs['banksy_leiden'] = pd.Categorical(banksy_mm_leiden.iloc[:, 0].astype(str))

sc.pl.embedding(adata, basis='banksy_umap', color='banksy_leiden',legend_loc="on data")
#%% banksy结果探索：哪个cluster搞不清楚就看哪个
meta = adata.obs.loc[adata.obs["leiden"] == "8", 
                     ["leiden", "rctd_inferred_celltype",'banksy_leiden']]
meta["rctd_inferred_celltype"].value_counts()
meta["banksy_leiden"].value_counts()
...
celltype.loc[celltype['ClusterID'].isin(['0','10']), 'celltype'] = 'Malignant_Plasma'
celltype.loc[celltype['ClusterID'].isin(['1']), 'celltype'] = 'Neutrophil'
celltype.loc[celltype['ClusterID'].isin(['2']), 'celltype'] = 'Erythoid'
celltype.loc[celltype['ClusterID'].isin(['3']), 'celltype'] = 'T/NK'
celltype.loc[celltype['ClusterID'].isin(['4']), 'celltype'] = 'Mono/Macro'
celltype.loc[celltype['ClusterID'].isin(['5']), 'celltype'] = 'Stromal'
celltype.loc[celltype['ClusterID'].isin(['6','11']), 'celltype'] = 'Endothelium'
celltype.loc[celltype['ClusterID'].isin(['7']), 'celltype'] = 'B'
celltype.loc[celltype['ClusterID'].isin(['8']), 'celltype'] = 'SOX2-OT+ Unknown'
celltype.loc[celltype['ClusterID'].isin(['9']), 'celltype'] = 'Megakaryocyte'
...
#%% 保存h5ad
adata.write_h5ad("10_mm_major_annotation.h5ad")