...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 22 20:03:20 2026
...
zarr_files = [
    "GSM9035027_MGUS-1_post_qc.zarr",
    "GSM9035028_MGUS-2_post_qc.zarr",
]
...
#%% 整合RCTD结果
rctd_mgus_1=pd.read_csv("./RCTD/doublet_result_mgus1.csv",index_col=0)
rctd_mgus_2=pd.read_csv("./RCTD/doublet_result_mgus2.csv",index_col=0)

rctd_mgus_1.index = "GSM9035027_MGUS-1_" + rctd_mgus_1.index.astype(str)
rctd_mgus_2.index = "GSM9035028_MGUS-2_" + rctd_mgus_2.index.astype(str)

rctd=pd.concat([rctd_mgus_1,rctd_mgus_2],axis=0)
# 把RCTD的结果写回去
...
celltype.loc[celltype['ClusterID'].isin(['0','2']), 'celltype'] = 'Neutrophil'
celltype.loc[celltype['ClusterID'].isin(['1']), 'celltype'] = 'Macrophage'
celltype.loc[celltype['ClusterID'].isin(['3']), 'celltype'] = 'Monocyte'
celltype.loc[celltype['ClusterID'].isin(['4','9','14','15']), 'celltype'] = 'Metamylomacyte'
celltype.loc[celltype['ClusterID'].isin(['5','6','8','11','12','13','21','24']), 'celltype'] = 'Erythoid'
celltype.loc[celltype['ClusterID'].isin(['7']), 'celltype'] = 'T'
#celltype.loc[celltype['ClusterID'].isin(['21']), 'celltype'] = 'Unknown'
# 21群之前未知，参考Banksy结果定Erythoid
celltype.loc[celltype['ClusterID'].isin(['10','19','20']), 'celltype'] = 'Stromal'
celltype.loc[celltype['ClusterID'].isin(['16']), 'celltype'] = 'Plasma'
celltype.loc[celltype['ClusterID'].isin(['17']), 'celltype'] = 'B'
celltype.loc[celltype['ClusterID'].isin(['18']), 'celltype'] = 'Megakaryocyte'
celltype.loc[celltype['ClusterID'].isin(['23']), 'celltype'] = 'Endothelium'
celltype.loc[celltype['ClusterID'].isin(['22']), 'celltype'] = 'Adipocyte'
...
#%% 整合banksy结果
banksy_mgus_pca=pd.read_csv("./MGUS_2_samples_PCA.csv",index_col=0)
banksy_mgus_umap=pd.read_csv("./MGUS_2_samples_UMAP.csv",index_col=0)
banksy_mgus_leiden=pd.read_csv("./MGUS_2_samples_leiden_clustering.csv",index_col=0)

adata.obsm['X_banksy_pca'] = banksy_mgus_pca.values.astype(np.float32) 
adata.obsm['X_banksy_umap'] = banksy_mgus_umap.values.astype(np.float32)
adata.obs['banksy_leiden'] = pd.Categorical(banksy_mgus_leiden.iloc[:, 0].astype(str))
...
meta = adata.obs.loc[adata.obs["leiden"] == "21", 
...
#%% RCTD结果探索：哪个cluster搞不清楚就看哪个
#meta = adata.obs.loc[adata.obs["leiden"] == "8", 
#                     ["leiden", "rctd_inferred_celltype"]]
#meta["rctd_inferred_celltype"].value_counts()
#%% 保存结果
adata.write_h5ad("2_mgus_major_annotation.h5ad")
