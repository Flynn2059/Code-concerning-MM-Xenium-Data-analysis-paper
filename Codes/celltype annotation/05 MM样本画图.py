...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  9 13:01:58 2026
...
groups = ["Other", "CD4+ T Naive", "CD8+ T Effector","CD8+ T Exhausted","T Proliferating"]
palette = ["#BDBDBD", "#00A087FF", "#DC0000FF","#F39B7FFF","#3C5488FF"]
...
for sdata_key, sdata in sdata_dict.items():
    ...
    adata.obs.loc[adata.obs["annotation"] == "CD8+ T Exhausted", "Tcell_highlight"] = "CD8+ T Exhausted"
...
for sample_name, sample_label in zip(sample_order, sample_labels):
    ...
    cd8_effector_mask = ann == "CD8+ T Effector"
    cd8_exhausted_mask = ann == "CD8+ T Exhausted"

    cd4_count = CD4_label_mask.sum()
    cd8_count = CD8_label_mask.sum()
    cd8_effector_count = cd8_effector_mask.sum()
    cd8_exhausted_count = cd8_exhausted_mask.sum()

    cd4_pct_total = cd4_count / total_cells * 100
    cd8_pct_total = cd8_count / total_cells * 100
    cd8_effector_pct_total = cd8_effector_count / total_cells * 100
    cd8_exhausted_pct_total = cd8_exhausted_count / total_cells * 100
    ...
    result_list.append({
        ...
        "CD4_count": cd4_count,
        "CD8_count": cd8_count,
        "CD8_Effector_count": cd8_effector_count,
        "CD8_Exhausted_count": cd8_exhausted_count,
        "CD4_pct_total": cd4_pct_total,
        "CD8_pct_total": cd8_pct_total,
        "CD8_Effector_pct_total": cd8_effector_pct_total,
        "CD8_Exhausted_pct_total": cd8_exhausted_pct_total
    })
...
cd8_effector_values = df_plot["CD8_Effector_pct_total"].values
cd8_exhausted_values = df_plot["CD8_Exhausted_pct_total"].values
...
ax.bar(
    x,
    cd8_exhausted_values,
    width=0.7,
    bottom=cd4_values + cd8_effector_values,
    label="CD8+ T Exhausted",
    color="#F39B7FFF"
)
...
plt.savefig(
    os.path.join(outdir, "MM_T_cell_barplot.pdf"),
    bbox_inches="tight"
)

plt.savefig(
    os.path.join(outdir, "MM_T_cell_barplot.svg"),
    bbox_inches="tight"
)
...
df_ratio.to_csv(
    os.path.join(outdir_ratio, "MM_CD4_CD8_ratio_table.csv")
)
...
for sdata_key, sdata in sdata_dict.items():
    ...
    # T 细胞 centroid
    t_mask = gdf["annotation"].isin(["CD4+ T Naive", "CD8+ T Effector","CD8+ T Exhausted","T Proliferating"])
...
plasma_labels = ["Malignant_Plasma"]
...
fig, axes = plt.subplots(2, 5, figsize=(25, 10), dpi=600)
axes = axes.flatten()
...
plt.tight_layout()
plt.savefig(
    os.path.join(outdir, "10_mm_T_vs_Plasma_2D_KDE_contour_panel.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, "10_mm_T_vs_Plasma_2D_KDE_contour_panel.svg"),
    bbox_inches="tight"
)
plt.close()
...
os.makedirs(outdir, exist_ok=True)
sample_order = [
    "sdata_GSM9035029_MM-1",
    "sdata_GSM9035030_MM-2",
    "sdata_GSM9035031_MM-3",
    "sdata_GSM9035032_MM-4",
    "sdata_GSM9035033_MM-5",
    "sdata_GSM9035034_MM-6",
    "sdata_GSM9035035_MM-7",
    "sdata_GSM9035036_MM-8",
    "sdata_GSM9035037_MM-9",
    "sdata_GSM9035038_MM-10",
]

fig, axes = plt.subplots(2, 5, figsize=(90, 40), dpi=600)
axes = axes.flatten()
...
plt.savefig(
    os.path.join(outdir, "MM_T_cell_spatial_co_localization_plot.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, "MM_T_cell_spatial_co_localization_plot.svg"),
    bbox_inches="tight"
)
plt.close()