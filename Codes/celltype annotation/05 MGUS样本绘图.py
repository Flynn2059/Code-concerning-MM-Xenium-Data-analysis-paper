...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  9 11:58:46 2026
...
annotation_level1["major_annotation"] = pd.Categorical(
    annotation_level1["major_annotation"],
...
groups = ["Other", "CD4+ T Central Memory", "CD8+ T Effector","CD8+ T Exhausted","T Proliferating"]
...
for sdata_key, sdata in sdata_dict.items():
    ...
    adata.obs.loc[adata.obs["annotation"] == "CD4+ T Central Memory", "Tcell_highlight"] = "CD4+ T Central Memory"
...
plt.savefig(
    os.path.join(outdir, "Mgus_T_cell_barplot.pdf"),
    bbox_inches="tight"
)

plt.savefig(
    os.path.join(outdir, "Mgus_T_cell_barplot.svg"),
    bbox_inches="tight"
)
...
df_ratio.to_csv(
    os.path.join(outdir_ratio, "Mgus_CD4_CD8_ratio_table.csv")
)
...
for sdata_key, sdata in sdata_dict.items():
    ...
    # T 细胞 centroid
    t_mask = gdf["annotation"].isin(["CD4+ T Central Memory", "CD8+ T Effector","CD8+ T Exhausted","T Proliferating"])
...
t_labels = ["CD4+ T Central Memory", "CD8+ T Effector","CD8+ T Exhausted","T Proliferating"]
...
plt.tight_layout()
plt.savefig(
    os.path.join(outdir, "2_mgus_T_vs_Plasma_2D_KDE_contour_panel.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, "2_mgus_T_vs_Plasma_2D_KDE_contour_panel.svg"),
    bbox_inches="tight"
)
plt.close()
...
os.makedirs(outdir, exist_ok=True)
sample_order = [
    "sdata_GSM9035027_MGUS-1",
    "sdata_GSM9035028_MGUS-2",
]
...
plt.savefig(
    os.path.join(outdir, "Mgus_T_cell_spatial_co_localization_plot.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, "Mgus_T_cell_spatial_co_localization_plot.svg"),
    bbox_inches="tight"
)
plt.close()