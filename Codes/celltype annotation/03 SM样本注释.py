...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar  1 21:24:51 2026
...
#%% 整合RCTD结果
rctd_sm_1=pd.read_csv("./RCTD/doublet_result_SM1.csv",index_col=0)
rctd_sm_2=pd.read_csv("./RCTD/doublet_result_SM2.csv",index_col=0)
rctd_sm_3=pd.read_csv("./RCTD/doublet_result_SM3.csv",index_col=0)
rctd_sm_4=pd.read_csv("./RCTD/doublet_result_SM4.csv",index_col=0)
rctd_sm_5=pd.read_csv("./RCTD/doublet_result_SM5.csv",index_col=0)

rctd_sm_1.index = "GSM9035039_SM-1_" + rctd_sm_1.index.astype(str)
rctd_sm_2.index = "GSM9035040_SM-2_" + rctd_sm_2.index.astype(str)
rctd_sm_3.index = "GSM9035041_SM-3_" + rctd_sm_3.index.astype(str)
rctd_sm_4.index = "GSM9035042_SM-4_" + rctd_sm_4.index.astype(str)
rctd_sm_5.index = "GSM9035043_SM-5_" + rctd_sm_5.index.astype(str)

rctd=pd.concat([rctd_sm_1,rctd_sm_2,rctd_sm_3,rctd_sm_4,rctd_sm_5],axis=0)
# 把RCTD的结果写回去
...
#%% 整合banksy结果
banksy_sm_pca=pd.read_csv("./SM_5_samples_PCA.csv",index_col=0)
banksy_sm_umap=pd.read_csv("./SM_5_samples_UMAP.csv",index_col=0)
banksy_sm_leiden=pd.read_csv("./SM_5_samples_leiden_clustering.csv",index_col=0)


adata.obsm['X_banksy_pca'] = banksy_sm_pca.values.astype(np.float32) 
adata.obsm['X_banksy_umap'] = banksy_sm_umap.values.astype(np.float32)
adata.obs['banksy_leiden'] = pd.Categorical(banksy_sm_leiden.iloc[:, 0].astype(str))
...
meta = adata.obs.loc[adata.obs["leiden"] == "3", 
...
celltype.loc[celltype['ClusterID'].isin(['0']), 'celltype'] = 'B'
celltype.loc[celltype['ClusterID'].isin(['1','4','10','12']), 'celltype'] = 'Mono/Macro'
celltype.loc[celltype['ClusterID'].isin(['2']), 'celltype'] = 'T/NK'
celltype.loc[celltype['ClusterID'].isin(['3']), 'celltype'] = 'Neutrophil'
celltype.loc[celltype['ClusterID'].isin(['5']), 'celltype'] = 'Endothelium'
celltype.loc[celltype['ClusterID'].isin(['6']), 'celltype'] = 'Neutrophil'
celltype.loc[celltype['ClusterID'].isin(['7']), 'celltype'] = 'Plasma'
celltype.loc[celltype['ClusterID'].isin(['8']), 'celltype'] = 'SOX2-OT+ Unknown'
celltype.loc[celltype['ClusterID'].isin(['9']), 'celltype'] = 'Erythoid'
celltype.loc[celltype['ClusterID'].isin(['11']), 'celltype'] = 'Fibroblast'
celltype.loc[celltype['ClusterID'].isin(['13']), 'celltype'] = 'HSC'
celltype.loc[celltype['ClusterID'].isin(['14']), 'celltype'] = 'Adipocyte'
celltype.loc[celltype['ClusterID'].isin(['15']), 'celltype'] = 'Megakaryocyte'
...
#%% 保存h5ad
adata.write_h5ad("5_sm_major_annotation.h5ad")