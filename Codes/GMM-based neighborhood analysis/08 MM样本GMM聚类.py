...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 12:43:17 2026
...
# 合并所有样本
all_samples_df = pd.concat(all_samples_df, axis=0)
all_samples_df.index = range(len(all_samples_df))
...
axes[0].hist(all_samples_df["t_norm"], bins=150)
axes[0].set_title("Pooled - t_norm")
axes[0].set_xlabel("t_norm")
axes[0].set_ylabel("Cell count")

axes[1].hist(all_samples_df["p_norm"], bins=150)
axes[1].set_title("Pooled - p_norm")
axes[1].set_xlabel("p_norm")
axes[1].set_ylabel("Cell count")

h = axes[2].hist2d(
    all_samples_df["t_norm"],
    all_samples_df["p_norm"],
    bins=150,
    range=[[0, 1], [0, 1]]
)
axes[2].set_title("Pooled - t_norm vs p_norm")
axes[2].set_xlabel("t_norm")
axes[2].set_ylabel("p_norm")
plt.colorbar(h[3], ax=axes[2], label="Cell count")
...
plt.tight_layout()
plt.savefig(
    os.path.join(outdir, "Pooled_t_norm_p_norm_peak_inspection.pdf"),
    bbox_inches="tight"
)
plt.savefig(
    os.path.join(outdir, "Pooled_t_norm_p_norm_peak_inspection.svg"),
    bbox_inches="tight"
)
plt.close()
...
all_cell_niche_df.to_csv(
    os.path.join(outdir, "MM_10_samples_cell_niche_matrix.csv"),
    index=False
)

print("All samples finished.")
print("Cell-level niche matrix saved:")
print(os.path.join(outdir, "MM_10_samples_cell_niche_matrix.csv"))
