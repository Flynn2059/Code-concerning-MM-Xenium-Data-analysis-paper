...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  2 12:52:04 2026
...
#%% 基于 GMM center 自动注释 cluster 类型 + 条件式人工确认 + Infiltrating Zone + 重绘 scatter/spatial 图
import os
import ast
import numpy as np
import pandas as pd
...
def build_cluster_annotation(centers, final_t_cluster, final_p_cluster):
    ...
    if str(final_t_cluster) == str(final_p_cluster):
        centers.loc[centers["cluster"] == str(final_t_cluster), "cluster_annotation"] = "Infiltrating Zone"
    else:
...
def make_legend_handles(categories_present):
    ordered_categories = ["T_Dominant", "Plasma_Dominant", "Mixed", "Infiltrating Zone"]
    handles = []
...
def process_one_sample_annotation(sdata_key, sdata):
    ...
    for n_components in n_components_list:
        ...
        for label in ["T_Dominant", "Plasma_Dominant", "Mixed", "Infiltrating Zone"]:
...
all_cell_niche_df.to_csv(
    os.path.join(outdir, "MGUS_2_samples_cell_niche_matrix.csv"),
    index=False
)

print("All samples finished.")
print("Cell-level niche matrix saved:")
print(os.path.join(outdir, "MGUS_2_samples_cell_niche_matrix.csv"))